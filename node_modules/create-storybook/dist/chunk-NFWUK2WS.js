import { readFile, appendFile, rm, mkdir, writeFile, stat } from 'node:fs/promises';
import { installableProjectTypes, detect, isStorybookInstantiated, ProjectType, detectLanguage, detectPnp, AngularJSON, promptForCompoDocs, CoreBuilder, compoDocPreviewPrefix, copyTemplate, SupportedLanguage, getBabelDependencies, copyTemplateFiles, detectBuilder, extractEslintInfo, configureEslintPlugin, getVersionSafe, externalFrameworks } from 'storybook/internal/cli';
import { JsPackageManagerFactory, versions, commandLog, paddedLog, HandledError, getProjectRoot, getPackageDetails } from 'storybook/internal/common';
import { withTelemetry } from 'storybook/internal/core-server';
import { GenerateNewProjectOnInitError, NxProjectDetectedError } from 'storybook/internal/server-errors';
...
import semver, { prerelease, lt, major } from 'semver';
...
`),installableProjectTypes.sort().forEach(framework=>paddedLog(`- ${framework}`)),logger4.log(),new HandledError(`Unknown project type supplied: ${projectTypeProvided}`);else try{projectType=await detect(packageManager,options);}catch(err){throw done(String(err)),new HandledError(err)}done();let storybookInstantiated=isStorybookInstantiated();if(options.force===!1&&storybookInstantiated&&projectType!==ProjectType.ANGULAR){logger4.log();let{force}=await prompts2([{type:"confirm",name:"force",message:"We found a .storybook config directory in your project. Therefore we assume that Storybook is already instantiated for your project. Do you still want to continue and force the initialization?"}]);logger4.log(),force?options.force=!0:process.exit(0);}options.skipInstall||await packageManager.installDependencies();let installResult=await installStorybook(projectType,packageManager,options);if(options.skipInstall||await packageManager.installDependencies(),options.disableTelemetry||await telemetry("init",{projectType}),projectType===ProjectType.REACT_NATIVE)return logger4.log(dedent`
    ...
    `),{shouldRunDev:!1};let foundGitIgnoreFile=await findUp(".gitignore"),rootDirectory=getProjectRoot();foundGitIgnoreFile&&foundGitIgnoreFile.includes(rootDirectory)&&((await readFile(foundGitIgnoreFile,"utf-8")).includes("*storybook.log")||await appendFile(foundGitIgnoreFile,`
*storybook.log
`));let storybookCommand=projectType===ProjectType.ANGULAR?`ng run ${installResult.projectName}:storybook`:packageManager.getRunStorybookCommand();return logger4.log(boxen(dedent`
        ...
        `,{borderStyle:"round",padding:1,borderColor:"#F1618C"})),{shouldRunDev:!!options.dev&&!options.skipInstall,projectType,packageManager,storybookCommand}}async function initiate(options){let initiateResult=await withTelemetry("init",{cliOptions:options,printError:err=>!err.handled&&logger4.error(err)},()=>doInitiate(options));if(initiateResult?.shouldRunDev){let{projectType,packageManager,storybookCommand}=initiateResult;logger4.log(`
Running Storybook`);try{let supportsOnboarding=[ProjectType.REACT_SCRIPTS,ProjectType.REACT,ProjectType.WEBPACK_REACT,ProjectType.REACT_PROJECT,ProjectType.NEXTJS,ProjectType.VUE3,ProjectType.ANGULAR].includes(projectType),flags=[];packageManager.type==="npm"&&projectType!==ProjectType.ANGULAR&&flags.push("--"),supportsOnboarding&&flags.push("--initial-path=/onboarding"),flags.push("--quiet"),packageManager.runPackageCommandSync(storybookCommand.replace(/^yarn /,""),flags,void 0,"inherit");}catch{}}}

export { doInitiate, initiate };
